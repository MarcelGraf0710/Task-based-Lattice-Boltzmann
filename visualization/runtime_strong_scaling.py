...
for access_pattern in access_patterns:
    ...
    model_tl = numpy.poly1d(numpy.polyfit(core_ticks, parallel_two_lattice_results, len(core_ticks)-2))
    model_tlf = numpy.poly1d(numpy.polyfit(core_ticks, parallel_two_lattice_framework_results, len(core_ticks)-2))
    model_ts = numpy.poly1d(numpy.polyfit(core_ticks, parallel_two_step_results, len(core_ticks)-2))
    model_shift = numpy.poly1d(numpy.polyfit(core_ticks, parallel_shift_results, len(core_ticks)-2))
    model_swap = numpy.poly1d(numpy.polyfit(core_ticks, parallel_swap_results, len(core_ticks)-2))
