...
)
{
    ...
    boundary_conditions::update_velocity_input_velocity_output(destination, access_function);
...
)
{
    ...
    for(auto time = 0; time < iterations; ++time)
    {
        ...
        result[time] = two_lattice_sequential::perform_tl_stream_and_collide_debug
        (
            fluid_nodes, 
            boundary_nodes, 
            source, 
